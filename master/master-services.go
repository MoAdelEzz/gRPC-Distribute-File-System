...
func (s *Master2ClientServer) SelectMachineToCopyTo(ctx context.Context, req *Services.SelectMachineRequest) (*Services.SelectMachineResponse, error) {
	...
	} else if FileTransferPort == "-2" {
		return nil, errors.New(Utils.NO_AVAILABLE_DATA_NODE)
	} else {
		RegisterFileTransferStart(machineIp, FileTransferPort, req.Filename)
		return &Services.SelectMachineResponse{MachineAddress: machineIp + ":" + FileTransferPort}, nil
	}
}
...
func (s *Master2ClientServer) GetSourceMachine(ctx context.Context, req *Services.GetSourceMachineRequest) (*Services.GetSourceMachineResponse, error) {
	...
	if ip == "" || port == "-1" {
		...
		println("Found file " + req.Filename + " on " + ip + ":" + port)
		return &Services.GetSourceMachineResponse{MachineAddress: ip + ":" + port}, nil
	}
}
